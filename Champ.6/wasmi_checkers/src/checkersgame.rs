...
impl CheckersGame {
    ...
    pub fn init(&mut self) -> Result<()> { // (1) A wrapper for the initBoard module function
        self.module_instance
            .invoke_export("initBoard", &[], &mut self.runtime)?;
        Ok(())
    }

    pub fn move_piece(&mut self, // (2) Converts co-ordinate tuples into simple i32s and calls move
                      from: &Coordinate,
    ...
    pub fn get_turn_owner(&mut self) -> Result<PieceColor> { // (3) Converts a numeric turn owner from the module to a more API-friendly enum(PieceColor)
        let res = self
            .module_instance
    ...
    pub fn get_board_contents(&mut self) -> Result<String> {    // to render the board contents as a string
