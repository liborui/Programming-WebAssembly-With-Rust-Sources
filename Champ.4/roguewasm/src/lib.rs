...
#[wasm_bindgen]
extern "C" {
    // With the log function, notice that you can import functions from specific JavaScript namespaces (e.g., console). 
    // You can tell wasm-bindgen which JavaScript module contains the function you’re going to import, as we do with the stats_updated function.
    fn alert(s: &str);
    ...
    // By declaring the Display type inside the extern block, wasm-bindgen makes that type available 
    // to your code and generates everything necessary to communicate with it.
    pub type Display;

    // By using the structural and method keywords in the wasm_bindgen macro, we are telling the macro and JavaScript 
    // boilerplate to take the method accessed on the Display type and call the decorated function in the WebAssembly module.
...
// To deal with something that could be missing like the prize location (you’ll write that code in a bit), we make use of Rust’s Option type.
#[wasm_bindgen]
pub struct Engine {
    display: Display,
